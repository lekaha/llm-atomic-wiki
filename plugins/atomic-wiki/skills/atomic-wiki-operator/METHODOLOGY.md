...
3. Write a short note describing each branch's definition and boundary (in this methodology notes)
...
These conventions make `atomic_wiki_lint` and `atomic_wiki_gen_index` work:
...
3. **First line must be `# title`** — `atomic_wiki_gen_index` reads this for page titles.
...
5. **Temporal markers in uniform format** — version as `v3.5`, date as `2025-04`, avoid "current" / "latest" / "now" in time-sensitive contexts, use specific dates (`as of 2025-04`). `atomic_wiki_lint` regex-checks these.
...
Build `index.md` at the wiki root (auto-generated by `atomic_wiki_gen_index`). Lists all wiki pages, organized by branch, one-line summary each. This is the LLM's entry point for Query — it doesn't read every page, it scans the index to decide which pages to load.

### Change log

`log.md`, append-only. Every Ingest or page update logs an entry. Use `atomic_wiki_append_log`.
...
4. Run `atomic_wiki_ingest` (which automatically runs `atomic_wiki_gen_index` and `atomic_wiki_append_log`)
...
**Programmatic Lint** (`atomic_wiki_lint`): checks ghost links, orphan pages, format violations, outdated markers. Seconds to run. Deterministic. Output: `lint-report.md`.
...
4. **Action**: decide which to fix, do the edits, run `atomic_wiki_append_log`
